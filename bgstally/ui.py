...
class UI:
    ...
    def get_prefs_frame(self, parent_frame: tk.Frame):
        ...
        tk.Button(frame, text=_("Configure Remote Server"), command=partial(self._show_api_window, parent_frame)).grid(row=current_row, column=1, padx=10, sticky=tk.W); current_row += 1 # LANG: Preferences button label        ttk.Separator(frame, orient=tk.HORIZONTAL).grid(row=current_row, columnspan=2, padx=10, pady=1, sticky=tk.EW); current_row += 1
        ...
        tk.Button(frame, text=_("Force Tick"), command=self._confirm_force_tick, bg="red", fg="white").grid(row=current_row, column=1, padx=10, sticky=tk.W); current_row += 1 # LANG: Preferences button label        # Add exploration settings
        current_row = self._add_exploration_settings(frame, current_row)
        return frame


    def _add_exploration_settings(self, frame, current_row):
        """Add exploration settings section to preferences."""
        ttk.Separator(frame, orient=tk.HORIZONTAL).grid(row=current_row, columnspan=2, padx=10, pady=1, sticky=tk.EW)
        current_row += 1
        
        # Exploration settings heading
        nb.Label(frame, text="Exploration Data Tracking", font=FONT_HEADING_2).grid(
            row=current_row, column=0, padx=10, sticky=tk.NW
        )
        current_row += 1
        ...
        nb.Checkbutton(
            frame, 
            text="Enable Exploration Data Transmission",
            variable=self.bgstally.state.ExplorationEnabled,
            onvalue=CheckStates.STATE_ON,
            offvalue=CheckStates.STATE_OFF,
            command=self._exploration_settings_changed
        ).grid(row=current_row, column=1, padx=10, sticky=tk.W)
        current_row += 1
        
        # API endpoint entry
        nb.Label(frame, text="Exploration API Endpoint URL").grid(
            row=current_row, column=0, padx=10, sticky=tk.W
        )
        EntryPlus(
            frame, 
            textvariable=self.bgstally.state.ExplorationApiEndpoint, 
            width=80
        ).grid(row=current_row, column=1, padx=10, pady=1, sticky=tk.W)
        current_row += 1
        
        # API key entry (optional)
        nb.Label(frame, text="Exploration API Key (Optional)").grid(
            row=current_row, column=0, padx=10, sticky=tk.W
        )
        EntryPlus(
            frame, 
            textvariable=self.bgstally.state.ExplorationApiKey, 
            width=80,
        ...
        nb.Label(
            frame, 
            text="When enabled, exploration events (scans, jumps, discoveries) will be sent to your API endpoint.",
            font=FONT_SMALL
        ).grid(row=current_row, columnspan=2, padx=10, sticky=tk.W)
        current_row += 1
        
        return current_row


    def _exploration_settings_changed(self):
        """Handle exploration settings changes."""
        self.bgstally.update_exploration_settings()
    def save_prefs(self):
        """
        Preferences frame has been saved (from EDMC core or any plugin)
        """
        # Update exploration settings when preferences are saved
        self.bgstally.update_exploration_settings()
