...
from bgstally.exploration_handler import ExplorationDataHandler
...
class BGSTally:
    ...
    def plugin_start(self, plugin_dir: str):
        ...
        self.exploration_handler: ExplorationDataHandler = ExplorationDataHandler(self)
        
        # Initialize exploration handler with current settings
        self.update_exploration_settings()
    ...
    def plugin_stop(self):
        ...
        """
        self.exploration_handler.stop_worker()
        self.ui.shut_down()
        self.save_data()
    ...
    def journal_entry(self, cmdr, is_beta, system, station, entry, state):
        ...
        # Process exploration events if enabled
        self.exploration_handler.journal_entry(cmdr, is_beta, system, station, entry, state)
    ...
    def update_exploration_settings(self):
        """
        Update exploration handler settings from state
        """
        if hasattr(self, 'exploration_handler') and hasattr(self, 'state'):
            self.exploration_handler.update_settings(
                enabled=self.state.ExplorationEnabled.get(),
                api_endpoint=self.state.ExplorationApiEndpoint.get(),
                api_key=self.state.ExplorationApiKey.get()
            )
